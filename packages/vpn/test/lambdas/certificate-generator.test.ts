...
describe('Certificate Generator Lambda', () => {
  ...
  it('should verify client certificate is signed by CA certificate', async () => {
    ...
    expect(clientCertificate.validity.notAfter.getTime()).toBeGreaterThan(
      clientCertificate.validity.notBefore.getTime()
    );
  });
  ...
  it('should handle unknown error types in catch block', async () => {
    ...
    const originalGenerateCACertificateAsync =
      require('../../lib/lambdas/certificate-generator/certificate-utils').generateCACertificateAsync;
    const certificateUtils = require('../../lib/lambdas/certificate-generator/certificate-utils');
    certificateUtils.generateCACertificateAsync = jest
      .fn()
      .mockImplementation(() => {
    ...
    certificateUtils.generateCACertificateAsync =
      originalGenerateCACertificateAsync;
  });
